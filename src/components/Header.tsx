...
export function Header() {
  ...
  return (
    <header
      className={`sticky top-0 border-b border-wood-100/80 bg-cream-50/95 backdrop-blur-xl ${menuOpen ? "z-[100]" : "z-40"}`}
    >
      ...
      <AnimatePresence>
        {menuOpen && (
          <>
            <m.div
              className="fixed inset-0 z-[100] bg-wood-950/75 backdrop-blur-md md:hidden"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              aria-hidden="true"
            ...
            <m.nav
              id="mobile-menu"
              className="fixed inset-y-0 right-0 z-[110] flex w-[min(88vw,320px)] flex-col border-l border-wood-200 bg-cream-50 shadow-2xl md:hidden"
            ...
            >
              <div className="flex items-center justify-between border-b border-wood-200 bg-cream-50 px-5 py-4">
                <span className="text-base font-bold text-wood-900">Menü</span>
                <button
                  type="button"
                  onClick={closeMenu}
                  className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-wood-200 bg-white text-wood-900 shadow-sm transition-colors hover:bg-wood-50"
              ...
              <ul className="flex-1 overflow-y-auto bg-cream-50 px-4 py-3">
                {NAV_LINKS.map((link) => (
                  <li key={link.href}>
                    <a
                      href={link.href}
                      onClick={closeMenu}
                      className="flex items-center rounded-xl px-4 py-3.5 text-base font-semibold text-wood-900 transition-colors hover:bg-wood-100 active:bg-wood-100"
                    >
                      {link.label}
                    </a>
                  </li>
              ...
              <div className="border-t border-wood-200 bg-cream-50 p-4">
                <a
